...
func (oc *OrderController) ShortPollOrders(c *gin.Context) {
	initialCount, err := oc.getAllOrders.Execute()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	c.Stream(func(w io.Writer) bool {
		select {
		case <-ticker.C:
			currentCount, err := oc.getAllOrders.Execute()
			if err != nil {
			...
			if currentLen != initialLen {
				c.SSEvent("message", gin.H{"mensaje": "El número de órdenes ha cambiado", "de": initialLen, "a": currentLen})
...
func (oc *OrderController) LongPollOrders(c *gin.Context) {
	initialCount, err := oc.getAllOrders.Execute()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	...
	for {
		...
		default:
			...
			currentLen := len(currentCount)
			if currentLen != initialLen {
				c.JSON(http.StatusOK, gin.H{"mensaje": "El número de órdenes ha cambiado", "de": initialLen, "a": currentLen})
				return
			}
